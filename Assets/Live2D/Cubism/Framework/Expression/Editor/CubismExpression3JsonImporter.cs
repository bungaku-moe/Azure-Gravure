...
namespace Live2D.Cubism.Editor.Importers
{
    public sealed class CubismExpression3JsonImporter : CubismImporterBase
    {
        ...
        /// </summary>
        [NonSerialized] private CubismExp3Json _expressionJson;

        private CubismExp3Json ExpressionJson
        {
            get
            {
                if (_expressionJson != null) return _expressionJson;
        ...
        {
            var oldExpressionData =
                AssetDatabase.LoadAssetAtPath<CubismExpressionData>(AssetPath.Replace(".exp3.json", ".exp3.asset"));
            ...
            var expressionListPath = Path.GetDirectoryName(directoryName).Replace("\\", "/") + "/" + modelName +
            ...
            if (expressionList.CubismExpressionObjects.Any(element => element == null))
            {
                ...
                var expressionDistinctObjectsArray =
                    cubismExpressionObjectsToList.Distinct(expressionEqualityComparer).ToArray();
        ...
        private static void OnModelImport(CubismModel3JsonImporter importer, CubismModel model)
        {
            ...
            if (expressionController == null || importer.Model3Json.FileReferences.Expressions == null) return;
