...
namespace Live2D.Cubism.Framework.Pose
{
    ...
    public sealed class CubismPoseController : MonoBehaviour, ICubismUpdatable
    {
        ...
        /// </summary>
        [SerializeField] public int defaultPoseIndex;
        ...
        public void Refresh()
        {
            ...
            for (var i = 0; i < tags.Length; ++i)
            {
                ...
                if (_poseData == null || _poseData.Length <= groupIndex) Array.Resize(ref _poseData, groupIndex + 1);
                ...
                if (partIndex != defaultPoseIndex) _poseData[groupIndex][partIndex].Part.Opacity = 0.0f;

                _poseData[groupIndex][partIndex].Opacity = _poseData[groupIndex][partIndex].Part.Opacity;

                if (tags[i].Link == null || tags[i].Link.Length == 0) continue;
        ...
        private void DoFade()
        {
            for (var groupIndex = 0; groupIndex < _poseData.Length; ++groupIndex)
            {
                ...
                // Fail silently...
                if (appearPartsGroupIndex < 0) return;

                // Delay disappearing parts groups disappear.
                for (var i = 0; i < _poseData[groupIndex].Length; ++i)
                {
                    // Fail silently...
                    if (i == appearPartsGroupIndex) continue;
                    ...
                    if (part.Opacity > delayedOpacity) part.Opacity = delayedOpacity;
                }
            }
        }
        ...
        private void CopyPartOpacities()
        {
            ...
            for (var partIndex = 0; partIndex < _poseData[groupIndex].Length; ++partIndex)
            {
                ...
                for (var linkIndex = 0; linkIndex < linkParts.Length; ++linkIndex)
                {
                    ...
                    if (linkPart != null) linkPart.Opacity = opacity;
                }
            }
        }
        ...
        {
            // Fail silently...
            if (!enabled || _model == null || _poseData == null) return;
