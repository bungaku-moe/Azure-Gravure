...
namespace Live2D.Cubism.Framework.Physics
{
    ...
    public class CubismPhysicsRig
    {
        ...
        ///     Children of rig.
        /// </summary>
        [SerializeField] public CubismPhysicsSubRig[] SubRigs;


        [SerializeField] public Vector2 Gravity = CubismPhysics.Gravity;


        [SerializeField] public Vector2 Wind = CubismPhysics.Wind;
        ...
        [NonSerialized] private float[] _parametersCache; // Cache parameters used by Evaluate.

        [NonSerialized] private float[] _parametersInputCache; // Cache input when UpdateParticles runs.

        public float[] ParametersCache
        {
            get => _parametersCache;
        ...
        public void Initialize()
        {
            ...
            for (var i = 0; i < SubRigs.Length; ++i) SubRigs[i].Initialize();
        }
        ...
        public void Stabilization()
        {
            ...
            // Initialize.
            if (_parametersCache == null) _parametersCache = new float[Controller.Parameters.Length];

            if (_parametersCache.Length < Controller.Parameters.Length)
                Array.Resize(ref _parametersCache, Controller.Parameters.Length);

            if (_parametersInputCache == null) _parametersInputCache = new float[Controller.Parameters.Length];
            ...
            // Evaluate.
            for (var i = 0; i < SubRigs.Length; ++i) SubRigs[i].Stabilization();
        ...
        {
            if (0.0f >= deltaTime) return;

            _currentRemainTime += deltaTime;
            if (_currentRemainTime > CubismPhysics.MaxDeltaTime) _currentRemainTime = 0.0f;
            ...
            if (_parametersInputCache.Length < Controller.Parameters.Length)
            {
                ...
                for (var i = 0; i < _parametersInputCache.Length; i++) _parametersInputCache[i] = _parametersCache[i];
            }
            ...
            while (_currentRemainTime >= physicsDeltaTime)
            {
                ...
                for (var i = 0; i < Controller.Parameters.Length; i++)
                {
                    _parametersCache[i] = _parametersInputCache[i] * (1.0f - inputWeight) +
                                          Controller.Parameters[i].Value * inputWeight;
                    _parametersInputCache[i] = _parametersCache[i];
                }

                for (var i = 0; i < SubRigs.Length; ++i) SubRigs[i].Evaluate(physicsDeltaTime);

                _currentRemainTime -= physicsDeltaTime;
            }

            var alpha = _currentRemainTime / physicsDeltaTime;
            for (var i = 0; i < SubRigs.Length; ++i) SubRigs[i].Interpolate(alpha);
        }
    }
}
