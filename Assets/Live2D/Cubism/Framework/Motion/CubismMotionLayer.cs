    ...
    public class CubismMotionLayer : ICubismFadeState
    {
        ...
        public void Update()
        {
            ...
            if (AnimationEndHandler == null || _playingMotions.Count != 1 || _isFinished
                || _motionState.ClipPlayable.GetDuration() == double.MaxValue ||
                Time.time <= _playingMotions[0].EndTime)
                return;
            ...
            {
                if (events[i].functionName != "InstanceId") continue;
        ...
        public static CubismMotionLayer CreateCubismMotionLayer(PlayableGraph playableGraph,
            CubismFadeMotionList fadeMotionList, int layerIndex, float layerWeight = 1.0f)
        {
        ...
        private CubismFadePlayingMotion CreateFadePlayingMotion(AnimationClip clip, bool isLooping, float speed = 1.0f)
        {
            ...
            if (isNotFound) Debug.LogError("CubismMotionController : Not found motion from CubismFadeMotionList.");
        ...
        {
            if (_motionState != null) _playableGraph.Disconnect(_motionState.ClipMixer, 0);
            ...
            if (_playingMotions.Count > 0 && _playingMotions[_playingMotions.Count - 1].Motion != null)
            {
                ...
                if (newEndTime < 0.0f || newEndTime < motion.EndTime) motion.EndTime = newEndTime;
        ...
        {
            for (var i = _playingMotions.Count - 1; i >= 0; --i) StopAnimation(i);
        }
