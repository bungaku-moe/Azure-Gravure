...
namespace Live2D.Cubism.Framework.MotionFade
{
    ...
    public class CubismFadeController : MonoBehaviour, ICubismUpdatable
    {
        ...
        /// </summary>
        [SerializeField] public CubismFadeMotionList CubismFadeMotionList;
        ...
        public void OnLateUpdate()
        {
            ...
            for (var i = 0; i < _fadeStates.Length; ++i)
            {
                ...
                if (playingMotions == null || playingMotions.Count <= 1) continue;
            ...
            for (var i = 0; i < _fadeStates.Length; ++i)
            {
                ...
                for (var j = playingMotions.Count - 2; j >= 0; --j)
                {
                    ...
                    if (time <= playingMotion.EndTime) continue;
        ...
        private void UpdateFade(ICubismFadeState fadeState)
        {
            ...
            if (playingMotions.Count > 0 && playingMotions[playingMotions.Count - 1].Motion != null &&
                playingMotions[playingMotions.Count - 1].IsLooping)
            {
            ...
            for (var i = 0; i < playingMotions.Count; i++)
            {
                ...
                for (var j = 0; j < DestinationParameters.Length; ++j)
                {
                    ...
                    {
                        if (fadeMotion.ParameterIds[k] != DestinationParameters[j].Id) continue;
                ...
                for (var j = 0; j < DestinationParts.Length; ++j)
                {
                    ...
                    for (var k = 0; k < fadeMotion.ParameterIds.Length; ++k)
                    {
                        if (fadeMotion.ParameterIds[k] != DestinationParts[j].Id) continue;
        ...
        {
            if (curve.length <= 0) return currentValue;
