...
namespace Live2D.Cubism.Editor.Inspectors
{
    ...
    /// </summary>
    [CustomEditor(typeof(CubismPart))]
    ...
    internal sealed class CubismPartInspector : UnityEditor.Editor
    {
        ...
        /// </summary>
        private bool IsInitialized => ChildParts != null && ChildPartDrawables != null;
        ...
        private void Initialize()
        {
            ...
            for (var i = 0; i < parts.Length; i++)
                ...
                {
                    Array.Resize(ref childParts, childParts.Length + 1);
        ...
        private class InternalTreeInfo
        {
            ...
            public InternalTreeInfo(CubismPart part, CubismDrawable[] srcDrawables, CubismPart[] srcParts,
                ref int drawableCount)
            {
                Part = part;
                ...
                for (var i = 0; i < srcDrawables.Length; i++)
                    ...
                    {
                        Array.Resize(ref Drawables, Drawables.Length + 1);
                ...
                for (var i = 0; i < srcParts.Length; i++)
                    if (srcParts[i].UnmanagedParentIndex == part.UnmanagedIndex)
                    {
                        ...
                        Childs[Childs.Length - 1] =
                            new InternalTreeInfo(srcParts[i], srcDrawables, srcParts, ref drawableCount);
                    }
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (ChildPartsFoldout) ChildParts.DoLayoutList();

            {
                ...
                ChildPartDrawables.Foldout =
                    EditorGUI.Foldout(foldoutRect, ChildPartDrawables.Foldout, "Child Drawables");
            ...
            if (ChildPartDrawables.Foldout)
            {
                ...
                for (var i = 0; i < ChildPartDrawables.Childs.Length; i++) DrawTree(ChildPartDrawables.Childs[i]);
                EditorGUI.indentLevel--;
            }
        }
        ...
        private void DrawTree(InternalTreeInfo item)
        {
            ...
            if (item.Foldout)
            {
                ...
                for (var i = 0; i < item.Childs.Length; i++) DrawTree(item.Childs[i]);
            }
        }
