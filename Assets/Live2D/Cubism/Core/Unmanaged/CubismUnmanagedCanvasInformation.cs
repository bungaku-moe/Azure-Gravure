...
namespace Live2D.Cubism.Core.Unmanaged
{
    ...
    public sealed class CubismUnmanagedCanvasInformation
    {
        ...
        {
            if (modelPtr == IntPtr.Zero) return;

            var canvasSize = new float[2];
            var canvasOrigin = new float[2];
            var pixelsPerUnitBuffer = new float[1];

            fixed (float* canvasSizePtr = canvasSize, canvasOriginPtr = canvasOrigin, pixelsPerUnitPtr =
                       pixelsPerUnitBuffer)
            {
                CubismCoreDll.ReadCanvasInfo(modelPtr, (IntPtr)canvasSizePtr, (IntPtr)canvasOriginPtr,
                    (IntPtr)pixelsPerUnitPtr);
